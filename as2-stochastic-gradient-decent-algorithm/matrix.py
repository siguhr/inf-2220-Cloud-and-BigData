...
class MF():
    ...
    def __init__(self, K, start, learning_rate, iterations, tolerance, copy):
        self.X = X
        self.num_users, self.num_items = X.shape
        self.K = K
        self.learning_rate = learning_rate
    ...
    def sgd(self):                                  # Stochastic Gradient Descent calculates the error and updates the model for each example in the training dataset. 
        ...
        for i, j, r in self.samples:                #At the i and j (x,y-cordination) there is a shuffled training value r = (data-point) 
            ...
            U_users_copy = self.P[i, :][:]
            
            
            # Update user and item latent feature matrices
            self.U_users[i, :] += (self.learning_rate * (e * self.V_items[j, :]))
            self.V_items[j, :] += (self.learning_rate * (e * U_users_copy))
...
def main():
    ...
    gradient_descent(X, np.random)

    #-  MF  -#
    iterations = 1000
    mf = MF(X, 2, 0.0000001, iterations, 1e-06,  copy)
    mf.train()
